...
| **Reversal Exit** | `REV_EXIT_*` in both traders, `--rev-exit` in sim_replay, exit_reason "Reversal exit" | Exit rule (live Jul 25 2026) | After peak ≥120pts: 2 consecutive adverse CLOSED 5-min bars + ≥30% of peak gone → exit at market and let the entry engine re-enter. Validated 2026 YTD +23% / 2025 OOS +20%. Sunset review Aug 24 2026. |
| **Partial Scale-Out** | `PARTIAL_TAKE_PTS=150`, `--partial` in sim_replay, exit_reason "Partial scale-out" | Exit rule (live Jul 25 2026) | On 2-contract trades: bank 1 contract at +150pts, runner's stop to breakeven. Partial rows are CLOSED rows tagged `notes='partial of #id'` — excluded from daily-entry counts and Trade Cop matching. |
